  ...
  <section>
    ...
    <section class="knowledge-table-section">
      ...
      <div class="table-wrap knowledge-table"><table><thead><tr><th>知识库名称</th><th>知识库描述</th><th>类型</th><th>上传文档</th><th>创建时间</th><th>操作</th></tr></thead><tbody>
        <tr v-for="library in pageItems" :key="library.id"><td><strong>{{ library.name }}</strong></td><td><span class="knowledge-description">{{ library.description }}</span></td><td><span class="knowledge-type" :class="library.knowledgeType">{{ library.knowledgeType === "image" ? "图片" : "文本" }}</span></td><td><label class="upload-btn">上传{{ library.knowledgeType === "image" ? "图片" : "文档" }}<input type="file" :accept="library.knowledgeType === 'image' ? 'image/*' : '.txt,.md,.pdf,.docx'" @change="upload(library.id, $event)"></label><small>{{ library.collectionCount }} 个文档</small></td><td>{{ library.createdAt }}</td><td><div class="row-actions"><button class="tiny-btn" @click="openDetail(library)">查看文档</button><button class="danger-btn" @click="removeLibrary(library)">删除</button></div></td></tr>
