...
export function configureAppController(options = {}) {
  invalidate = options.invalidate || invalidate;
  navigate = options.navigate || navigate;
}
...
const portalNav = {
  ...
  government: {
    title: "政府入口",
    subtitle: "Government Workspace",
    home: "/government/dashboard",
    items: [["/government/dashboard", "政府驾驶舱", "Government"]]
  },
  alliance: {
    title: "联盟入口",
...
function portalFor(path) {
  if (path.startsWith("/talent")) return portalNav.talent;
  if (path.startsWith("/government")) return portalNav.government;
  if (path.startsWith("/alliance")) return portalNav.alliance;
  return portalNav.enterprise;
}

export function renderApp(path = currentPath) {
  currentPath = path;
  ...
  return route?.layout === "full" ? content : shell(path, content);
}
  ...
  return `
    <div class="app-shell">
      <aside class="side-nav">
        ${brand(portal)}
        <nav>
          ${portal.items.map(([href, label, en]) => `
      ...
      <main class="main-area">
        <header class="top-bar">
          ...
          <div class="top-actions">
            <button class="ghost-btn" data-go="/">切换入口</button>
            ${portal === portalNav.enterprise ? `<button class="primary-btn" data-go="/enterprise/tools">AI工具中心</button>` : `<button class="primary-btn" data-go="${portal.home}">当前首页</button>`}
          </div>
        </header>
...
function brand(portal = portalNav.enterprise) {
  return `<button class="brand" data-go="${portal.home}"><span class="yapmitai-wordmark compact">YAPMITAI</span><small>${portal.subtitle}</small></button>`;
}
...
function handlePrimaryClick(event) {
  ...
  if (action === "close-drawer") {
    ...
    saveModelConfig();
  } else if (action === "close-tool-run") {
    state.activeTool = null;
    state.toolRunError = "";
    render();
  } else if (action === "run-active-tool") {
    runActiveTool();
  } else if (action === "open-tool-create") {
    openToolDrawer();
  } else if (action === "close-tool-drawer") {
    closeToolDrawer();
  } else if (action === "save-tool") {
    saveTool();
  }
}
...
function handleSecondaryClick(event) {
  ...
  const runToolButton = event.target.closest("[data-tool-run]");
  if (runToolButton) {
    openToolRun(runToolButton.dataset.toolRun);
    return;
  }

  const editToolButton = event.target.closest("[data-tool-edit]");
  if (editToolButton) {
    const tool = state.tools.find((item) => String(item.id) === editToolButton.dataset.toolEdit);
    openToolDrawer(tool);
    return;
  }

  const deleteToolButton = event.target.closest("[data-tool-delete]");
  if (deleteToolButton) {
    deleteTool(deleteToolButton.dataset.toolDelete);
  }
}

export function handleAppChange(event) {
...
async function initializeTools() {
  await loadToolChatModels();
  await loadTools();
}

async function loadToolChatModels() {
  try {
    const response = await toolsApi.chatModels();
    state.toolChatModels = response.data || [];
    if (currentPath === "/enterprise/tools") render();
  } catch (error) {
    state.toolError = error.message;
  }
}

async function loadTools() {
  try {
    const params = state.toolFilter === "全部" ? {} : { category: state.toolFilter };
    const response = await toolsApi.list(params);
    state.tools = response.data || [];
    if (state.activeTool) {
      state.activeTool = state.tools.find((item) => item.id === state.activeTool.id) || state.activeTool;
    }
    state.toolError = "";
    if (currentPath === "/enterprise/tools") render();
  } catch (error) {
...
function openToolRun(id) {
  state.activeTool = state.tools.find((item) => String(item.id) === String(id));
  state.toolRunError = "";
  render();
}

async function runActiveTool() {
  const task = document.getElementById("tool-run-task")?.value.trim();
  const modelConfigId = Number(document.getElementById("tool-run-model")?.value || state.activeTool?.modelConfigId || 0);
  if (!state.activeTool || state.toolRunBusy) return;
  if (!task) {
    state.toolRunError = "请填写任务简报。";
    render();
    return;
  }
  state.toolRunBusy = true;
  state.toolRunError = "";
  render();
  try {
    const response = await toolsApi.run(state.activeTool.id, {
      task,
      model_config_id: modelConfigId || null
    });
    const record = response.data;
    state.tools = state.tools.map((tool) => tool.id === state.activeTool.id
      ? {
          ...tool,
          callCount: (tool.callCount || 0) + 1,
          recentRecords: [record, ...(tool.recentRecords || [])].slice(0, 3)
        }
      : tool
    );
    state.activeTool = state.tools.find((tool) => tool.id === state.activeTool.id);
  } catch (error) {
    state.toolRunError = error.message;
  } finally {
    state.toolRunBusy = false;
    render();
  }
}

function openToolDrawer(tool = null) {
  state.toolDrawerOpen = true;
  state.toolForm = tool ? { ...tool } : {
    name: "",
    nameEn: "",
    code: "",
    category: "内容生成",
    description: "",
    icon: "技",
    modelConfigId: state.toolChatModels[0]?.id || "",
...
function readToolForm() {
  return {
    ...
    name_en: document.getElementById("tool-name-en")?.value.trim() || null,
    ...
    icon: document.getElementById("tool-icon")?.value.trim() || null,
    model_config_id: Number(document.getElementById("tool-model-config")?.value || 0) || null,
    prompt_template: document.getElementById("tool-prompt-template")?.value.trim(),
    input_schema: { fields: [{ name: "task", label: "任务简报", type: "textarea" }] },
    output_schema: { type: "object", fields: ["title", "target", "suggested_action", "deliverables"] },
    enabled: document.getElementById("tool-enabled")?.checked,
    is_system: document.getElementById("tool-is-system")?.checked,
    sort_order: Number(document.getElementById("tool-sort-order")?.value || 0)
  };
}

async function saveTool() {
  const payload = readToolForm();
  const editingId = state.toolForm?.id;
  if (!payload.name || !payload.code || !payload.category || !payload.prompt_template) {
    state.toolFormError = "请填写技能名称、唯一编码、分类和 Prompt 模板。";
    render();
    return;
  }
  try {
    ...
    state.toolFormError = error.message;
    render();
  }
}

async function deleteTool(id) {
  const tool = state.tools.find((item) => String(item.id) === String(id));
  const name = tool?.name || "该AI工具";
  if (!window.confirm(`确定删除“${name}”吗？相关运行历史也会被删除。`)) return;
  try {
    await toolsApi.delete(id);
    if (state.activeTool?.id === Number(id)) state.activeTool = null;
    await loadTools();
  } catch (error) {
    state.toolError = error.message;
    render();
...
async function loadKnowledgeLibraries() {
  try {
    ...
    if (currentPath === "/enterprise/knowledge/agent") render();
  } catch {
...
async function loadModelConfigs() {
  try {
    ...
    if (currentPath === "/enterprise/model-configs") render();
  } catch (error) {
