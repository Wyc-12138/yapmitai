...
CREATE TABLE IF NOT EXISTS `ai_teams` (
    `id` INT NOT NULL AUTO_INCREMENT COMMENT '团队主键',
    ...
    `updated_at` DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6)
        ON UPDATE CURRENT_TIMESTAMP(6) COMMENT '更新时间',
    PRIMARY KEY (`id`),
    UNIQUE KEY `uq_ai_teams_name` (`name`)
) ENGINE=InnoDB
  DEFAULT CHARACTER SET=utf8mb4
  COLLATE=utf8mb4_unicode_ci
  COMMENT='AI团队';
...
-- ============================================================
-- 12. AI团队与AI员工关联表（多对多中间表）
-- 一个团队多个员工，一个员工多个团队
-- ============================================================
CREATE TABLE IF NOT EXISTS `ai_team_agents` (
    `id` INT NOT NULL AUTO_INCREMENT COMMENT '主键',
    `team_id` INT NOT NULL COMMENT '团队ID',
    `agent_id` INT NOT NULL COMMENT 'AI员工ID(对应agents表id)',
    `created_at` DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) COMMENT '加入时间',
    PRIMARY KEY (`id`),
    UNIQUE KEY `uq_team_agent` (`team_id`, `agent_id`),
    KEY `ix_ai_team_agents_team_id` (`team_id`),
    KEY `ix_ai_team_agents_agent_id` (`agent_id`),
    CONSTRAINT `fk_ai_team_agents_team`
        FOREIGN KEY (`team_id`)
        REFERENCES `ai_teams` (`id`)
        ON DELETE CASCADE
        ON UPDATE CASCADE,
    CONSTRAINT `fk_ai_team_agents_agent`
        FOREIGN KEY (`agent_id`)
        REFERENCES `agents` (`id`)
        ON DELETE CASCADE
        ON UPDATE CASCADE
